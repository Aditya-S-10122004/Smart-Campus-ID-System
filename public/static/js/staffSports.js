// public/static/js/staffSports.js
(function () {
  ...
  function prependVisit(student) {
    ...
    item.innerHTML = `
      ...
      <div class="sm-visit-middle">${
        student.indoor_sports_active === true
          ? "Subscribed"
          : student.indoor_sports_active === false
          ? "Not Subscribed"
          : student.category || ""
      }</div>
  ...
  async function sendProbe(blob) {
    try {
      ...
      const res = await fetch("/api/staff/sports/scan", {
        method: "POST",
        body: fd,
    ...
    if (result.matched) {
      ...
      if (result.recentVisit) {
        ...
        prependVisit({
          ...
          indoor_sports_active: result.student.indoor_sports_active,
  ...
  window._staffSports = { startScanning, stopScanning };
})();
